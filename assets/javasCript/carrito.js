  ...
  .then((res) => {
    ...
    for (const menos of operadorMenos) {
      menos.addEventListener("click", () => {
        ...
        if (cuadroEncontrado) {
          // <-- validar si se encontró un cuadro válido
...
function crearEventos(eventoSolo) {
  ...
  let btnClase = listaCarrito.some((prod) => prod._id == eventoSolo._id)
    ? "btn-warning"
    : "";
  return `
  <div class="card-carrito">
  <div class="col-md-4">
    <img src="${eventoSolo.imagen}" class="img-carrito" alt="img">
  </div>
  <div class="card-body">
    ...
    <div class="div-precioBoton">
      ...
      <div class="cantidades">
        <input class="resta operador simple-button" type="button" value="-" id="${eventoSolo._id}">
        <input class="cantidad inputCantidad simple-button" type="number" value="0" id="${eventoSolo._id}">
        <input class="suma operador mas simple-button" type="button" value="+" id="${eventoSolo._id}">
      </div>
      <button class="quitar btn simple-button ${btnClase}" data-id="${eventoSolo._id}" id="${eventoSolo._id}" onclick=borrarNota("${eventoSolo._id}")>Quitar del
